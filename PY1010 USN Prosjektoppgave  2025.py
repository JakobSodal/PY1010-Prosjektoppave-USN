...
u_dag = data['Ukedag'] .values # kolonne 1: ukedag for tlf-samtalen (Mandag osv.)
...
varighet_timedelta = pd.to_timedelta(data['Varighet'])

# Summerer varighetene
total_varighet = varighet_timedelta.sum()

# Konverter til sekunder
henvendelser_i_sekunder = varighet_timedelta.dt.total_seconds()
...
gjennomsnitt_varighet_timedelta = pd.to_timedelta(gjennomsnittlig_varighet_sekunder, unit='s')
# Fjern hundredelene
gjennomsnitt_varighet_timedelta_round = gjennomsnitt_varighet_timedelta .floor('s')


print('gjennomsnittlig tlf-samtaletid var:', gjennomsnitt_varighet_timedelta_round)
...
negativ = (score >= 1) & (tilfredshet <= 6) # 1-6 = negativ
nøytral = (score >= 7) & (tilfredshet <= 8) # 7-8 = nøytral
positiv = (score >= 9) & (tilfredshet <= 10) # 9-10 positiv
